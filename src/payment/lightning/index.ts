...
export class XyoLightningPayment extends XyoBase {
  ...
  constructor(store: IXyoPaymentStore) {
    ...
    this.server.start()
    this.logInfo(
      `Webhook server ready at url: http://localhost:${options.port}/`
    )
  ...
  private handler = async (request: Request, reply: ResponseToolkit) => {
    ...
    if (status === 'paid') {
      ...
      const currentCredits =
        (await this.store.getCreditsForKey(this.changeIdToApiKey[paymentid])) ||
        0
      const amountToBeAdded = this.changeIdToAmount[paymentid]
      this.logInfo(
        `Adding ${amountToBeAdded} credit(s) to ${this.changeIdToApiKey[paymentid]} `
      )
      if (!(await this.store.didSpend(this.changeIdToApiKey[paymentid]))) {
        this.store.spent(this.changeIdToApiKey[paymentid])
        this.store.setCreditsForKey(
          this.changeIdToApiKey[paymentid],
          currentCredits + amountToBeAdded
        )
      }
    }
    return true
